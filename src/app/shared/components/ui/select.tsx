    ...
    return (
      <div ref={containerRef} className={cn('relative', className)}>
        <button
          ...
          className={cn(
            'flex h-10 w-full items-center justify-between gap-2 rounded border border-slate-200 bg-white px-3 py-2 text-base focus:border-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-2',
        ...
        {open && (
          <ul
            role="listbox"
            className="absolute z-10 mt-1 max-h-72 w-full overflow-auto rounded border border-slate-200 bg-white py-1 shadow-md"
          >
            {options.map((option) => (
              <li key={option.value}>
                <button
                  ...
                  className={cn(
                    'flex w-full items-center px-3 py-2 text-left text-base hover:bg-slate-50',
