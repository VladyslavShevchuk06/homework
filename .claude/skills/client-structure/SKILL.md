...
description: Use when adding a module/widget/feature/entity/api/model slice to this Next.js app, adding a shared util/hook/store/service/system, registering a `(web)` route or an `(api)` route handler, touching the data layer (Drizzle schema, migration, seed, a query) or auth (Better Auth server instance, session check, route gating in `src/proxy.ts`), deciding where a new file should live, moving an existing slice to a different layer (re-homing it because a sibling imports it, or because it gained or lost consumers), or auditing the codebase against its Feature-Sliced Design (Layer/Slice/Segment) layout. Also use when bootstrapping a Next.js client from zero. Even if the user doesn't say "FSD" or "structure" explicitly — if they're placing new code anywhere under `src/`, use this skill. Skip for one-line edits, bug fixes, and refactors inside an existing slice.
---
...
## Two halves — a GENERIC core and a PROJECT appendix

This skill has a structural core that must stay generic, plus a small appendix that deliberately
documents **this** project. Know which half you are editing.

**The GENERIC core** — `SKILL.md`, `references/structure.md`, `references/pitfalls.md`,
`references/bootstrap.md`, `references/comments.md`, `spec/`, `examples/`, `scripts/`. It states
the pattern for ANY Next.js + React client, so every rule must read the same in a project it has
never seen. An edit here must NOT introduce:

- a real slice, file, symbol, route, env var or endpoint name — write `<module>`, `<widget>`,
  `<entity>`, `<x>.store.ts`, `E<Entity>Key` instead;
- a domain-specific word standing in for the pattern (a chat, a paywall, a checkout, a driver, a
  catalog) — name the ROLE the pattern gives it (the module, the modal widget, the api slice);
...
The **one exception inside the core** is the "Server side" section below: this project's server
lives in the same Next.js app as its client, so the layer rules have to say what a route handler
and the data layer may do. It names real folders on purpose.

**The PROJECT appendix** — `references/data-layer.md`, `references/auth.md`,
`references/state-management.md`. These document the concrete stack (Drizzle over Postgres,
Better Auth, TanStack Query v5 patterns) and **should** cite real paths, real env vars and real
file names. That is what makes them useful; genericising them would empty them. Keep them
accurate against the code, and when the code moves, move the path in the doc with it.

When a real run teaches something structural, generalise it before it lands in the core: state the
SHAPE and the mechanism, not the case. A fact that is only true here goes in the appendix or in
the project wiki, never in the core.
...
└── proxy.ts                                        # the edge gate — one file at src root
```
...
Thirteen rules that hold across every Next.js client built with this skill. Each rule has a one-line statement here; the reference docs own the detail.
...
7. **`pkg/*` self-containment, and the slot is entered through a barrel** — a `pkg/*` slot never imports from `app/*` or from another `pkg/*`; each folder must be liftable into another project as one folder. If two pkg slots need the same helper, duplicate it as a private file inside each pkg. The slot is a CLOSED unit: outsiders enter through `pkg/<slot>/index.ts`, not by reaching at a member file. When members are bound to different RUNTIMES, split them into `client/` and `server/` folders that publish their own barrels — see "A pkg slot may split by runtime". Detail under "Layer responsibilities".
...
9. **Env access through `config/env/` only** — never read `process.env` directly outside `config/env/env.client.ts` and `config/env/env.server.ts` (the `proxy.ts` boundary edge cases are noted in `references/pitfalls.md`).
10. **File size cap — 1000 lines** — no implementation file exceeds 1000 lines. When a file approaches the cap, split along the pattern: extract `elements/` sub-components, a `*.service.ts`, a `*.constant.ts`, or a new slice. A file already over the cap is split FIRST, then edited.
...
#### A `pkg` slot is a CLOSED unit — and it may split by RUNTIME
...
**A slot barrel is NOT a grouping barrel, even when its members are independent.** Rule 5
deletes a barrel whose folder merely groups unrelated units — but a `pkg/<slot>/` folder is
a UNIT by definition: it is the thing you lift into another project, and its `index.ts` is
the boundary that makes lifting possible. A generic barrel checker cannot tell the two
apart, because both look like a folder of unrelated members; when one flags a slot barrel,
that is a false positive to filter, not debt to pay. The test that distinguishes them is
ownership, not contents: deleting a grouping barrel changes nothing but import paths,
while deleting a slot barrel destroys the slot's public face.

The barrel cannot always be one file, because a slot's members are not always reachable
from the same runtime. A framework that renders on both the server and the client
resolves some dependencies differently per runtime — a package may expose a
server-condition build that omits what the browser build exports, and credentials or
native modules must never enter a browser graph at all. A single barrel that re-exports
everything creates ONE module edge to every member, so importing an isomorphic type from
it drags the runtime-bound members in too, and the build breaks in whichever environment
cannot resolve them.

**Group members by runtime and give each group its own barrel:**

```
pkg/<slot>/
  <slot>.<suffix>.ts   # isomorphic — safe in any runtime
  client/
    <name>.<suffix>.ts
    index.ts           # entered as `pkg/<slot>/client`
  server/
    <name>.<suffix>.ts
    index.ts           # entered as `pkg/<slot>/server`
  index.ts             # the isomorphic surface
```

Three barrels, one per runtime reach — so a consumer still never names a member file, and
"enter through the barrel" holds. Which barrel `index.ts` re-exports is the one judgement
call:

| The runtime folder's graph is | The slot barrel should |
|---|---|
| types and light values that resolve identically everywhere | `export * from './client'` / `'./server'` — one entry point stays simplest |
| a runtime-only dependency, an SDK, credentials, a native module | NOT re-export it — consumers enter `pkg/<slot>/client` or `pkg/<slot>/server` directly |

When the barrel deliberately omits a runtime folder, **say so in a comment at the top of
`index.ts`**, naming the folder and the mechanism. Otherwise the next person "fixes" the
missing re-export and reintroduces the break — this omission looks like an oversight and
is not one.
...
- **A runtime folder is not a role folder.** `client/` and `server/` group by *where the
  code runs*, and they DO carry a barrel — unlike `services/` or `hooks/`, which group by
  role and carry none (rule 5). Do not collapse a runtime split into a role folder, and
  do not invent a runtime folder for members that run anywhere.

**`src/proxy.ts`** — the Next.js edge gate (named `middleware.ts` before Next 16) lives at the **root of `src/`**, not under `app/`. One file. Composes locale routing, auth gates, session cookies, request-header rewrites. May read from `config/env/` and the relevant `pkg/<auth>/` (and other `pkg/` slots), but should not pull from modules/widgets/features.
...
| `proxy.ts` | the Next.js edge gate | `src/` root |
...
4. **Sort members by runtime.** Anything that only resolves in one runtime — a browser-only SDK, a server-only credential or native module — goes in `client/` or `server/` with its own `index.ts`; isomorphic members stay at the slot root. Consumers enter `pkg/<name>`, `pkg/<name>/client` or `pkg/<name>/server` — never a member file.
5. **Decide what the slot barrel re-exports.** Re-export a runtime folder only when its graph resolves everywhere (types, light values). When you omit one, comment at the top of `index.ts` saying which folder and why, or the omission reads as an oversight and gets "fixed".
...
3. If gating behaviour changes (new private path, new redirect rule), update `src/proxy.ts` accordingly — keep all gating in that one file. (Next 16 renamed `middleware.ts` to `proxy.ts`; the role is unchanged.)
4. For an `(api)` handler, follow the four-step contract in "Server side" below — `await connection()`, validate with Zod, enforce its own session check, delegate to the entity's `index.server` barrel.

## Server side — `(api)` handlers, the data layer, auth

This project has no separate backend: the server is the same Next.js deployment. So the layer rules
have to say what the server half may do. This is the one section of the core that names real
folders (see "Two halves" above); the *why* behind each stack choice lives in the appendix
(`references/data-layer.md`, `references/auth.md`).

### The runtime split inside an entity api slice

An `entities/api/<api>/` slice serves **both** runtimes, so it publishes **two barrels** — the same
"split by runtime, one barrel per runtime" rule the `pkg/` slots follow:

```
src/app/entities/api/<api>/
├── <api>.api.ts             # CLIENT fetcher — calls `/api/<route>`, no db import
├── <api>.query.ts           # CLIENT TanStack queryOptions factory
├── <api>.mutation.ts        # CLIENT 'use client' mutation hooks
├── <api>.query.server.ts    # SERVER queryOptions for page prefetch
├── <api>.service.ts         # SERVER 'server-only' + Drizzle — the ONLY place SQL is written
├── <api>.action.ts          # SERVER action (optional)
├── index.ts                 # client-safe barrel
└── index.server.ts          # server-only barrel
```

- **`*.service.ts` starts with `import 'server-only'`.** That import is the guard: pulling the
  service into a client graph becomes a build error instead of a leaked connection string.
- **The two barrels are not interchangeable.** A route handler or a page imports
  `@/app/entities/api/<api>/index.server`; a component imports `@/app/entities/api/<api>`.
  Importing the server barrel from a client component is the mistake this split exists to prevent.
- **SQL lives in exactly one file per slice** — the service. Handlers do not build queries, and
  neither do pages, modules or widgets.

### `(api)` route handler contract

`src/app/(api)/api/<route>/route.ts` exports `GET` / `POST` / … and does exactly four things:

1. **`await connection()` before touching request data** — with `cacheComponents: true` this is
   what opts the handler into dynamic rendering. Skipping it is a silent build-time failure mode.
2. **Validate input with Zod at the boundary.** Query params use `.catch(<default>)` per field so a
   malformed value degrades to the default instead of 400-ing the page.
3. **Enforce its own session check** when the data is user-scoped — `auth.api.getSession({ headers })`
   → 401. It never assumes `src/proxy.ts` already ran; `proxy.ts` gates *pages*, and `/api/*` falls
   through to the handler.
4. **Delegate to the entity's server barrel** and return `NextResponse.json(...)`. No query
   building, no business rules, no domain logic in the handler itself.

A handler is a routing file: like `page.tsx`, it declares nothing of its own beyond its Zod schema
and the exported verbs. Shared handler logic moves into the entity service, not into a sibling
handler.

### Data layer

Drizzle over Postgres (hosted on Supabase) is the only persistence path. `supabase-js` is **not**
used for data access and must not be added for it — Supabase is the Postgres instance, not the
client SDK. Aggregates use `db.$count(...)`; never count by fetching rows and reading `.length`.
Schema, migration and seed workflow: `references/data-layer.md`.

### Auth

Two halves, deliberately separate: the **server instance** (`betterAuth(...)` + `drizzleAdapter`)
is app-specific and server-only; the **client adapter** (`createAuthClient`) is liftable and lives
in the `pkg/auth/` slot. Route gating for pages lives in one file, `src/proxy.ts` (Next 16's name
for `proxy.ts`) — never scattered into pages. Detail: `references/auth.md`.

### Where the server-side folders belong — target state

Two folders predate these rules and sit outside the layer tree. The pattern's answer for each,
to be applied as a deliberate refactor (not silently while doing something else):

| Now | Target | Why |
|---|---|---|
| `src/db/` | `src/app/shared/systems/db/` | It is one subject with internals (client + schema + migration/seed scripts), specific to this application, and therefore not a `pkg/` — a `pkg` slot must be liftable into another project unchanged, and an app's own table schema never is. `systems/` is the fallback home for exactly this shape. Moving it also updates `drizzle.config.ts` and the `db:*` / `seed` scripts. |
| `src/lib/` | `src/app/shared/systems/auth/` | Same reasoning: the Better Auth server instance is wired to this project's schema, so it is a system, not a `pkg`. The name `lib` is rejected on its own terms — it collides with the liftable-integration folder (`pkg/`) and means nothing specific. The client adapter stays in `pkg/auth/`. |
| `src/db/favorites-count.ts` | its entity's `*.service.ts` | It is a query expression, not infrastructure. SQL belongs in the owning entity service. |

Until that refactor lands, the appendix documents the folders where they actually are — the docs
follow the code, not the plan.
...
node scripts/check-barrels.mjs --root src --ignore <generated-path>
```
...
| Importing a `pkg` member file from outside the slot (`pkg/<name>/<member>`) | The slot is closed. Enter `pkg/<name>`, or its runtime barrel `pkg/<name>/client` / `pkg/<name>/server`. |
...
This SKILL is the router: it decides which resource to open for the situation. The four resource sets are independent — they do **not** reference one another.
...
| **Checking** the two rules no toolchain sees (import direction, barrels) | `scripts/` — run both checkers |
...
| Writing a **query, mutation or optimistic update** (TanStack Query v5 patterns) | `references/state-management.md` |
| Touching the **database** — schema, migration, seed, aggregates, filters | `references/data-layer.md` |
| Touching **auth** — server instance, session check, OAuth env, route gating | `references/auth.md` |

Generic core:
- **`references/structure.md`** — Layer/Slice/Segment decision trees + isolation rules.
- **`references/bootstrap.md`** — Mode A new-project scaffold (`create-next-app`, `tsconfig`, ESLint, Prettier, Tailwind v4, `next-intl`, env validation, first route/module/api slice, the route-gating file).
...
- **`examples/`** — canonical file shapes per layer with `<…>` / `__…__` placeholders. `examples/proxy.ts` is the Next 16 route-gating shape.
- **`scripts/`** — zero-dependency checkers: `check-layer-imports.mjs`, `check-barrels.mjs`.

Project appendix (cites real paths on purpose — see "Two halves"):
- **`references/data-layer.md`** — Drizzle over Postgres/Supabase: persistence contract, `db.$count` aggregates, filter composition, schema + seeding workflow.
- **`references/auth.md`** — Better Auth: server instance, client adapter, optional OAuth env vars, server-side session reads, `src/proxy.ts` gating.
- **`references/state-management.md`** — TanStack Query v5: query keys, pagination, the optimistic mutation pattern, cross-cache synchronization, server/client boundaries.
