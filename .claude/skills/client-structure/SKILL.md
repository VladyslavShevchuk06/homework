...
│       └── index.ts
├── lib/                                            # project-wired framework singletons (not liftable as pkg)
│   └── <name>.ts                                   # e.g. Better Auth server instance
└── proxy.ts                                        # Next.js 16 edge gating (one file at src root)
```
...
5. **Env access through `config/env/` only** — never read `process.env` directly outside `config/env/env.client.ts` and `config/env/env.server.ts` (the `proxy.ts` boundary edge cases are noted in `references/pitfalls.md`).
...
3. If gating behaviour changes (new private path, new redirect rule), update `src/proxy.ts` accordingly — keep all gating in that one file.
...
- **`references/bootstrap.md`** — Mode A new-project scaffold (`create-next-app`, `tsconfig`, ESLint, Prettier, Tailwind v4, `next-intl`, env validation, first route/module/api slice, `proxy.ts`).
- **`references/comments.md`** — comment-style convention.
- **`references/pitfalls.md`** — common mistakes with explanations.
- **`references/data-layer.md`** — Drizzle/Postgres persistence contract, `db.$count` aggregations, the supabase-js prohibition.
- **`references/state-management.md`** — TanStack Query v5: query keys, `keepPreviousData` pagination, the optimistic mutation flow, cross-cache sync.
- **`references/auth.md`** — Better Auth server/client split, `src/proxy.ts` route protection, optional OAuth env vars, server session checks.
